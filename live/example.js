...
function resizeWin(){
	...
	var n_height_max = win.css("max-height").replace(/[^-\d\.]/g, ''); if(n_height > n_height_max)n_height = n_height_max;
	var question = $(".question");
	var answer = $(".answer");
	var header_height = 35;//as defined in the style sheet 
	var footer_height = 30;//as defined in the style sheet 
	var progressBar_height = 30;
	//var controls_height = 50;//as defined in the style sheet 
	var remaining_height = n_height - (header_height + footer_height + progressBar_height );
	
	if(n_height > n_width){
		question.css({"height":remaining_height / 2 +"px"});		
		
		question.css({"top":header_height+"px"});
		answer.css({"top":remaining_height / 2 + header_height +"px"});
		
		question.css({"bottom":"auto"});
		if(ouicards.settings["mode"]=='s') answer.css({"bottom":footer_height + progressBar_height +"px"});
		else answer.css({"bottom":footer_height +"px"});
		...
		if(ouicards.settings["mode"]=='s'){
			question.css({"bottom":footer_height+progressBar_height+"px"});
			answer.css({"bottom":footer_height+progressBar_height+"px"});
		}else{
			question.css({"bottom":footer_height+"px"});
			answer.css({"bottom":footer_height+"px"});
		}
